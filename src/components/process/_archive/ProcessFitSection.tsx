...
interface ProcessFitSectionProps {
  eyebrow?: string;
  heading?: string;
  goodItems?: string[];
  notItems?: string[];
...
export default function ProcessFitSection({ eyebrow, heading, goodItems, notItems }: ProcessFitSectionProps) {
      ...
      <Container>
        <AnimatedSection>
          <SectionHeading
            ...
            heading={heading ?? "Is This Right for You?"}
          />
        </AnimatedSection>
              ...
              <ul className="space-y-3">
                {goodFit.map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    <svg className="mt-0.5 h-5 w-5 shrink-0 text-sgwx-check" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                      <path d="M6 10l3 3 5-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    <span className="text-sm leading-relaxed text-sgwx-text">{item}</span>
                  </li>
                ))}
              </ul>
            </Card>
          ...
          <AnimatedSection delay={0.18}>
            <Card hover={false} className="h-full">
              ...
              <ul className="space-y-3">
                {notFit.map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    <svg className="mt-0.5 h-5 w-5 shrink-0 text-sgwx-cross" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
